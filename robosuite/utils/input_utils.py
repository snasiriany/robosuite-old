...
def input2action(device, robot, active_arm="right", env_configuration=None):
    ...
    if controller.name == 'IK_POSE':
        ...
        try:
            _ = SpaceMouse
            spacemouse_exists = True
        except:
            spacemouse_exists = False
        drotation = drotation * 50 if spacemouse_exists and isinstance(device, SpaceMouse) else drotation * 1.5
        dpos = dpos * 125 if spacemouse_exists and isinstance(device, SpaceMouse) else dpos * 75
    else:
