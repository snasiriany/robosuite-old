"""
Custom utils that leverage a modified set of features from robosuite
"""
...
class CustomDataCollectionWrapper(Wrapper):
	def __init__(self, env, directory, collect_freq=1, flush_freq=100, record_episode_numbers=True):
		"""
		...
		Args:
			...
			record_episode_numbers (bool): If True, will use episode numbering when creating episode directories
		...
		# Set counter to keep track of which episode we're at
		self.ep_ctr = -1

		# Whether we should name directories according to episode numbers
		self.record_episode_numbers = record_episode_numbers
	...
	def _on_first_interaction(self):
		...
		# create directory name based on argument
		dir_name = f"ep{self.ep_ctr}" if self.record_episode_numbers else f"ep_{t1}_{t2}"
		self.ep_directory = os.path.join(self.directory, dir_name)
	...
	def _flush(self):
		...
		### SOROUSH: added the line. Not sure if it should be added....
		self.has_interaction = False
	...
	def step(self, action, **kwargs):
		...
		ret = self.env.step(action, **kwargs)
		self.t += 1
...
class ReducedObsWrapper(Wrapper):
	"""
	Simple wrapper to reduce the observation space of the underlying env.

	Args:
		env (MujocoEnv): Robosuite environment to wrap
		keep_keys (dict): Modalities mapped to specific key(s) to keep as part of each corresponding obs space.
			For example, "proprio" modality might map to ["eef_pos", "eef_quat", "gripper_qpos"] to keep only those
			three values. Note that @keep_keys can simply be substrings of the full observation name; e.g.: "eef_pos"
			will still register "robot0_eef_pos" and keep this key, and "proprio" modality will register
			"robot0_proprio"
	"""
	...
	def __init__(self, env, keep_keys):
		...
		# Store keys to keep
		assert isinstance(keep_keys, dict), "Inputted keep_keys should be a dictionary mapping " \
											"modalities to observation keys!"
		self.keep_keys = keep_keys
		# Make sure only eef pos proprio obs are active
		for obs_name in self.env.active_observables:
			found, set_inactive = False, True
			for modality, modality_keys in self.keep_keys.items():
				if modality in self.env._observables[obs_name].modality:
					found = True
					for key in modality_keys:
						# Don't make this observable inactive if the key includes one that we want to keep
						if key in obs_name:
							set_inactive = False
							break
					if set_inactive:
						# Otherwise, set this observable to be inactive
						self.env.modify_observable(obs_name, "active", False)
				# Restart loop if we've already found (and processed) the key
				if found:
					break